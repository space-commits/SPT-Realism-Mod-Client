...
namespace RealismMod
{
    public static class PluginConfig
    {
        ...
        public static void InitConfigBindings(ConfigFile config)
        {
            ...
            CamMulti = config.Bind<float>(recoilSettings, "Camera Recoil Multi", 0.6f, new ConfigDescription("Visual Camera Recoil.", new AcceptableValueRange<float>(0f, 5f), new ConfigurationManagerAttributes { Order = 10, Browsable = Plugin.ServerConfig.recoil_attachment_overhaul }));
            ...
            CamReturn = config.Bind<float>(advancedRecoilSettings, "Camera Recoil Speed", 0.037f, new ConfigDescription("Higher = More Faster Camera Recoil", new AcceptableValueRange<float>(0f, 0.5f), new ConfigurationManagerAttributes { Order = 132, Browsable = Plugin.ServerConfig.recoil_attachment_overhaul }));
            ...
            DeviceVolume = config.Bind<float>(zoneSettings, "Device Volume", 0.1f, new ConfigDescription("Volume Modifier For Geiger And Gas Analyser.", new AcceptableValueRange<float>(0f, 2f), new ConfigurationManagerAttributes { Order = 10, Browsable = Plugin.ServerConfig.enable_hazard_zones }));
            GasMaskBreathVolume = config.Bind<float>(zoneSettings, "Gas Mask Breath Volume", 0.2f, new ConfigDescription("Volume Modifier For Gas Mask SFX.", new AcceptableValueRange<float>(0f, 2f), new ConfigurationManagerAttributes { Order = 20, Browsable = Plugin.ServerConfig.enable_hazard_zones }));
            ...
            ShowRadEffects = config.Bind<bool>(zoneSettings, "Show Visual Noise For Radiation", true, new ConfigDescription("Radiation Causes Visual Noise, The Strength Of Which Depends On Current Radiation Rate And Total Radiation Poisoning.", null, new ConfigurationManagerAttributes { Order = 40, Browsable = Plugin.ServerConfig.enable_hazard_zones }));
            MuteGasAnalyserKey = config.Bind(zoneSettings, "Mute Gas Analysed Key", new KeyboardShortcut(KeyCode.M, new[] { KeyCode.LeftControl }), new ConfigDescription("", null, new ConfigurationManagerAttributes { Order = 50, Browsable = Plugin.ServerConfig.headset_changes }));
            MuteGeigerKey = config.Bind(zoneSettings, "Mute Geiger Key", new KeyboardShortcut(KeyCode.M, new[] { KeyCode.LeftAlt }), new ConfigDescription("", null, new ConfigurationManagerAttributes { Order = 60, Browsable = Plugin.ServerConfig.headset_changes }));
