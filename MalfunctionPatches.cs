    ...
    public class GetTotalMalfunctionChancePatch : ModulePatch
    {
        ...
        private static bool Prefix(ref float __result,BulletClass ammoToFire, Player.FirearmController __instance, float overheat, out double durabilityMalfChance, out float magMalfChance, out float ammoMalfChance, out float overheatMalfChance, out float weaponDurability)
        {
            ...
            if (!player.IsAI)
            {
                ...
                {
                    durabilityMalfChance = (Math.Pow((double)(__instance.Item.BaseMalfunctionChance + 1f), 3.0 + (double)(100f - weaponDurability) / (20.0 - 10.0 / Math.Pow((double)__instance.Item.FireRate / 10.0, 0.322))) - 1.0) / 1000.0;
                }
                else
                {
                    durabilityMalfChance = (Math.Pow((double)(__instance.Item.BaseMalfunctionChance + 1f), Math.Log10(Math.Pow((double)(101f - weaponDurability), (50.0 - Math.Pow((double)weaponDurability, 1.286) / 4.8) / (Math.Pow((double)__instance.Item.FireRate, 0.17) / 2.9815 + 2.1)))) - 1.0) / 1000.0;
                }
