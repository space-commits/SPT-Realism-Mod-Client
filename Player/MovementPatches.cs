    ...
    public class SprintAccelerationPatch : ModulePatch
    {
        ...
        private static bool Prefix(MovementContext __instance, float deltaTime)
        {
            ...
            if (player.IsYourPlayer)
            {
                ...
                float sprintInertia = Mathf.Max(EFTHardSettings.Instance.sprintSpeedInertiaCurve.Evaluate(Mathf.Abs((float)EFT.MovementContext._averageRotationX.Average)), EFTHardSettings.Instance.sprintSpeedInertiaCurve.Evaluate(2.1474836E+09f) * (2f - player.Physical.Inertia));
